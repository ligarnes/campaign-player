...
public abstract class AuthorizableBasicBeans extends BasicBeans implements Serializable {
	...
	// Attention: Comme les elements sont des set, les methodes fireVetoableChange et firePropertyChange
	// 				ne permettent pas de seulement ajouter ou enelver un element (le set en entier sera 
	//              retransmis) et donc pour l 'instant rient n'est fait pour avertir avertir le remote
	//              des changement effetctuer sur ces variables. Et donc TODO trouver un moyen de faire
	...
	public void removeOwner(ClientId cliendId) throws AuthorizationManagerException{
		if (owners.size() <= 1) {
			throw new AuthorizationManagerException(String.format("Client id %s is the last owner of this object, owners set must contain at least one client id.", cliendId.toString()));
		}
		else {
