...
public class ShapeListGenerator implements ComboListFromXML {
	...
	public ArrayList<ArrayList<JPanel>> getShapeBuilders(String path,
			ArrayList<ArrayList<String[]>> name) throws InstantiationException,
		...
		for (int i = 0; i < name.size(); i++) {
			...
			for (int j = 0; j < name.get(i).size(); j++) {
				for (int k = 0; k < name.get(i).get(j).length; k++) {
					result.get(i).add(
							(JPanel) (getShapeBuilderClass(path, name.get(i)
									.get(j)[k]).newInstance()));
				}
			}
		}
