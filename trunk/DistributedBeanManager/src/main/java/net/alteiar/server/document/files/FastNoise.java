...
public class FastNoise {
	...
	public static BufferedImage generateImage(int width, int height) {
		BufferedImage noiseImage = new BufferedImage(width, height,
	...
	public static int noise(double x, double y, int nbOctave) {
		...
		while (octave != 0) {
			...
			alt1 = (grad1X * sX1 + grad1Y * sY1) + 16384
					+ ((alt1 & 0xFF0000) >> 9); // to avoid seams to be 0 we use
												// an offset
			alt2 = (grad2X * sX2 + grad2Y * sY2) + 16384
					+ ((alt2 & 0xFF0000) >> 9);
			alt3 = (grad3X * sX3 + grad3Y * sY3) + 16384
					+ ((alt3 & 0xFF0000) >> 9);
			alt4 = (grad4X * sX4 + grad4Y * sY4) + 16384
					+ ((alt4 & 0xFF0000) >> 9);
